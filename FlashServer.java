		...
		try {
			...
			while(this.alive){
				...
				if (line.length()>0) {
						...
						*/
						...
						for(String m:msgs){
							...
							if(msg[0].equals("Hello")){
								...
								this.player = new Player(Integer.parseInt(msg[1]),msg[2],Integer.parseInt(msg[3]),0,0,msg[5],this.output,Integer.parseInt(msg[4]));
								...
								if(!matched){
									...
									if(shouldQueue&&!queued){
										...
										if((assault&&FlashServer.queue1.size()==0)||(!assault&&FlashServer.queue2.size()==0)&&!queued){
											if(assault)
												FlashServer.queue1.add(player);
											else FlashServer.queue2.add(player);
											queued=true;
										}else if(!queued){
											gs = new GameServer(4481+87*FlashServer.games.size());
											gs.p2=this.player;
											if(assault)
												gs.p1=FlashServer.queue1.poll();
											else gs.p1=FlashServer.queue2.poll();
											t1="FoundYouAGame,localhost,"+gs.port+","+0+",4480\n";
											toSend+="\nFoundYouAGame,localhost,"+gs.port+","+0+",4480\n";
											sendAfter=1;
										...
										if(code.length()==0){
											...
											code="B2GARB"+code;
											gs = new GameServer(4479+87*FlashServer.privateMatches.size());
											gs.p1=this.player;
											...
											if(FlashServer.privateMatches.containsKey(joinCode)){
												...
												//
												toSend+="\nFoundYouAGame,localhost,"+FlashServer.privateMatches.get(joinCode).port+","+joinCode+",4480\n";
												gs = FlashServer.privateMatches.get(joinCode);
												t1="FoundYouAGame,localhost,"+FlashServer.privateMatches.get(joinCode).port+","+joinCode+",4480\n";
												gs.p2=this.player;
												sendAfter=1;
												gs.p1.doubleWrite(t1);
					...
					//	timeouts-=2;
					timeouts++;
					if(queued&&shouldQueue&&(FlashServer.queue1.contains(this.player)||FlashServer.queue2.contains(this.player))){
						timeouts--;
					}
					if(timeouts>10){
						doubleWrite("ServerMessage");
...
class GameServer extends Thread{
	...
	public void run(){
		...
		while (this.alive) {
			...
			if(g1!=null&&g2!=null&&g1.r&&g2.r){
				try{
					if(round==1){
						String welcome = FlashServer.encode("Welcome to Flash Private Server! Please report any bugs to Glenn M#9606 on discord, and use !help for a list of commands.");
	...
	public String toString(){
		if(g1.win==1){
			return g2.player.name+","+g2.player.id+","+"WIN\n"+g1.player.name+","+g1.player.id+","+"LOSE\nRound: "+(round-1)+", Approx time: "+g1.time/60+"m"+g1.time%60+"s, Cause: "+g1.reason;
		}else if(g2.win==1){
			return g1.player.name+","+g1.player.id+","+"WIN\n"+g2.player.name+","+g2.player.id+","+"LOSE\nRound: "+(round-1)+", Approx time: "+g2.time/60+"m"+g2.time%60+"s, Cause: "+g2.reason;
		}
		return null;
	}
}
		...
		try{
			...
			//	System.out.println("GST alive");
				...
				if(state==0){
					...
					doubleWrite("FoundYourGame,"+opponent.id+","+opponent.name+","+opponent.bs+","+opponent.decal+","+opponent.profile+","+this.seed+","+this.side+","+this.map+","+opponent.w+","+opponent.l);
					state=1;
				}if(this.player==null){
					doubleWrite("GimmeUrPlayerInfo,h");
				}
				if(lastTest!=null&&((new Date()).getTime()-lastTest.getTime())>5000){
					doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("Lag test results: The game ran at "+((((double)((int)(1.0/(((double)((new Date()).getTime()-lastTest.getTime()))/((double)(time*1000-lastTime*1000)))*10000)))/100.0))+"% speed over about 5 seconds"));
					lastTest=null;
				}
				try {
					char[] buffer=new char[1024];
						...
						if(msg[0].equals("HeresMyPlayerInfo")){
							...
							if(msg[0].equals("IChangedMyTowerLoadout")){
								...
								if(tail.contains("Cerem")){
									...
									opponent.doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("they WILL die,"));
								}
								...
								if(msg.length>1&&msg[1].equals("SentChatMsg")){
									try{
										...
										String param=null;
										if(cmd.length>1)
											param = FlashServer.decode(msg[2]).trim().substring(FlashServer.decode(msg[2]).trim().indexOf(' ')+1);
										if(cmd[0].equals("!help"))
											...
											doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode(FlashServer.getHistory(player.id)));
										else if(cmd[0].equals("!source"))
											doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("not available yet"));
										else if(cmd[0].equals("!profile")){
											if(cmd.length==1)
												doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("Current profile picture: "+FlashServer.getProfile(player.id)));
											else{
												doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("Set profile picture to: "+param));
											...
											if(param==null)
												count=4;
											else count = Math.min(Integer.parseInt(param),10);
											...
											doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode(x.toString().substring(1,x.toString().length()-1)));
											...
											doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("Known issues:\n-game disconnects sometimes\n-wins and losses aren't shown\n-wins, losses, battlescore don't update"));
										else if(cmd[0].equals("!lagtest")){
											doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("Starting lag test..."));
											lastTest=new Date();
											lastTime=this.time;
										}else
											toSend+=m;
									}catch(Exception e){
										doubleWrite("RelayMsg,SentChatMsg,"+FlashServer.encode("An error occurred while processing a chat message or command."));
										e.printStackTrace();
									}
								}else toSend+=m;
				...
				if(toSend.length()>0&&state==0)doubleWrite(toSend);
...
public class FlashServer {
	...
	public static String encode(String data){
		...
		try{
			...
			out.write(x);
			out.finish();
			String q=new String(Arrays.copyOfRange(out1.toByteArray(),2,out1.toByteArray().length),StandardCharsets.UTF_8);
			out1.close();
			out.close();
			return q;
		}catch(IOException e){return null;}//not possible
		...
		for(String x:privateMatches.keySet())
			if(!privateMatches.get(x).alive){
				...
				else entries1 = new ArrayList<String>(history.get(id1));
				if(entries1.size()>9)
		...
		for(String e:entries){
			...
			compact.add(lines[i+3]+"|win|"+lines[i+2]+"|"+"Opponent: "+lines[i+1].substring(0,lines[i+1].indexOf(','))+'\n'); 
			}else if(lines[i+1].contains(""+uid)){
				compact.add(lines[i+3]+"|lose|"+lines[i+2]+"|"+"Opponent: "+lines[i].substring(0,lines[i].indexOf(','))+'\n'); 
			}
		}
